...
export function validateConfig(config: WalbucketConfig): WalbucketConfig {
  ...
  if (gasStrategy === 'user-pays' && !config.userSigner) {
    throw new ConfigurationError(
      'userSigner is required when gasStrategy is "user-pays"'
    );
  }
  ...
  return {
    ...
    packageId,
    walrusPublisherUrl: config.walrusPublisherUrl || DEFAULT_WALRUS_URLS[network].publisher,
    walrusAggregatorUrl: config.walrusAggregatorUrl || DEFAULT_WALRUS_URLS[network].aggregator,
